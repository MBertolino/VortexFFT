...
int main() {
  ...
  // Generate circle
  for (int j = 0; j < M; j++) {
    x[2*j] = cos(TWOPI*j/(double)M) - 1.5;
    x[2*j+1] = 5*sin(TWOPI*j/(double)M);
    
    x[2*j+2*M] = cos(TWOPI*j/(double)M) + 1.5;
    x[2*j+1+2*M] = 5*sin(TWOPI*j/(double)M);
  ...
  for (int k = 0; k <= T; k++) {
    ...
    */
    ...
    N_old  = N;
    points_reloc(px, t, n, &M, &N, kappa, mu, gamma, beta);
    printf(" \n");
    printf("N_old = % d     N = %d    M = %d\n", N_old, N, M);
    size = N*n_dim;
