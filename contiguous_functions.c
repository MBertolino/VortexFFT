                 ...
                 double* beta, double* gamma)
{
  ...
  //kappa[start+1] = 2.*(t[0]*t[start+1] - t[start+1]*t[start])\
    /sqrt(kappa_den[0]*kappa_den[0] + kappa_den[1]*kappa_den[1]);   
 ...
 //  printf("kappaKULA = %e, j = %d \n", kappa[j], j);
  ...
  for (int j = start; j < N-1; j++) {
   ...
   // printf("mu = %e, beta = %e, gamma = %e, j = %d \n", mu[j], beta[j], gamma[j], j);
  }
